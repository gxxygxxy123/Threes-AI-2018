...
public:
	...
	data info() const { if(attr < 0 || attr > 4){std::cerr<<"debug: Error!"<<std::endl;} return attr; }
	data info(data dat) { if(attr < 0 || attr > 4 || dat < 0 || dat > 4){std::cerr<<"debug: Error!"<<std::endl;} data old = attr; attr = dat; return old; }
